...
import { AlertTriangle, OctagonX, Pause, Play, Square } from 'lucide-react';
...
export function AutonomyControlBar({ autonomy, onAction }: AutonomyControlBarProps) {
  ...
  return (
    <section className={styles.controlBar} aria-label="Autonomous video assembly controls">
      ...
      <div className={styles.controls} role="group" aria-label="Lifecycle controls">
        <button type="button" className={`${styles.controlBtn} ${styles.controlBtnPrimary}`} disabled={unavailable} onClick={() => onAction('start')}>
        ...
        <button type="button" className={`${styles.controlBtn} ${styles.controlBtnDanger}`} disabled={unavailable} onClick={() => onAction('emergency_stop')}>
          <OctagonX size={14} aria-hidden /> Emergency Stop
        </button>
      </div>
      {unavailable && autonomy.unavailableReason ? <p className={styles.srOnly}>{autonomy.unavailableReason}</p> : null}
      {unavailable ? (
